<?php
    ...
    try {
        ...
        // Cek apakah database esteh tersedia
        $mysqli->select_db("esteh");
        if ($mysqli->error){
            throw new Exception();
        }

        // Cek apakah tabel pesanan_esteh tersedia
        $query = "SELECT 1 FROM pesanan_esteh";
        $mysqli->query($query);
        if ($mysqli->error){
            throw new Exception();
        }
...
<html lang="id">
    <head>
        ...
        <title>App Pemesanan Es Teh</title>
    ...
    <div class="container" class="py-5">
        <div class="row">
            ...
            <h3 class="mt-5">
                Selamat datang di Aplikasi <strong>Pemesanan Es Teh</strong>
            </h3>
            <p class="lead mt-5">Sistem kami mendeteksi database / tabel belum tersedia, apakah ingin dibuat sekarang?</p>
                <a href="<?= $base_url ?>/database/migration.php" class="btn btn-info">Ya</a>
